...
public class ReloadCommand {
    public static int run(CommandContext<ServerCommandSource> context) {
        ...
        if (source.getPlayer() != null) {
            source.getPlayer().sendMessage(Text.literal("§6[LethalBreed] §aConfig reloaded! §e" + count + " zombies updated."));
        } else {
            source.sendFeedback(Text.literal("[LethalBreed] Config reloaded! " + count + " zombies updated."), false);
        }
