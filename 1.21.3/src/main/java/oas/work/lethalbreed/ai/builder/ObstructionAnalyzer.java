...
public class ObstructionAnalyzer {
    ...
    private static class CachedBlock {
        ...
        CachedBlock(World world, BlockPos pos, BlockState state, long tick) {
            this.state = state;
            this.tick = tick;
            this.blocking = !state.isAir() && (state.shouldBlockVision(world, pos) || !state.getCollisionShape(world, pos).isEmpty() || state.isFullCube(world, pos));
        }
    }
    ...
    public static boolean isBlocking(World world, BlockPos pos) {
        ...
        if (BLOCK_CACHE.size() < MAX_CACHE_SIZE) {
            BLOCK_CACHE.put(pos, new CachedBlock(world, pos, state, tick));
        }
