...
public class MovementCoordinator {
    public static boolean tryBuild(Zombie z, Vec3 t) {
        ...
        BlockPos next = BlockPos.containing(z.getX() + dir.x, z.getY() - 0.5, z.getZ() + dir.z);
        if (next.equals(curr)) return false;

        BlockPos floor = (t.y < z.getY() - 0.5) ? next.below(2) : next.below();
        if (w.getBlockState(floor).canBeReplaced() && VoidAnalyzer.isDeep(w, floor)) {
            z.getNavigation().stop(); z.setDeltaMovement(0, 0, 0); 
            center(z, curr);
