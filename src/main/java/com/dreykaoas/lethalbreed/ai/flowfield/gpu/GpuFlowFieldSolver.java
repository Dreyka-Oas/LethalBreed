...
final class GpuFlowFieldSolver {
    ...
    static FlowField solve(GpuContext ctx, Snapshot s) {
        ...
        cl_mem flagsMem = clCreateBuffer(ctx.context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
        ...
        try {
            ...
            clSetKernelArg(ctx.kernel, 2, Sizeof.cl_mem, Pointer.to(dirXMem));
            clSetKernelArg(ctx.kernel, 3, Sizeof.cl_mem, Pointer.to(dirZMem));
            clSetKernelArg(ctx.kernel, 4, Sizeof.cl_mem, Pointer.to(flagsMem));
            ...
            clSetKernelArg(ctx.kernel, 7, Sizeof.cl_mem, Pointer.to(changedMem));
