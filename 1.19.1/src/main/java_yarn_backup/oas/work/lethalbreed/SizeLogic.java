...
public class SizeLogic {
    ...
    public static void randomizeStats(ZombieEntity zombie, boolean allowMutant) {
        ...
        double hBase = zombie.getAttributeBaseValue(EntityAttributes.GENERIC_MAX_HEALTH);
        double finalHealth = hBase * scale * healthBonus;
        setAttribute(zombie, EntityAttributes.GENERIC_MAX_HEALTH, finalHealth);
        zombie.setHealth((float) finalHealth);

        double dBase = zombie.getAttributeBaseValue(EntityAttributes.GENERIC_ATTACK_DAMAGE);
        setAttribute(zombie, EntityAttributes.GENERIC_ATTACK_DAMAGE, dBase * scale * healthBonus);

        setAttribute(zombie, EntityAttributes.GENERIC_MOVEMENT_SPEED, speed);
    }

    private static void setAttribute(ZombieEntity zombie, net.minecraft.entity.attribute.EntityAttribute attr, double value) {
    ...
    public static void reapplyStats(ZombieEntity zombie) {
        if (zombie.getScoreboardTags().contains("lethal_mutant")) {
            return;
        }
        ...
        double speed = zombie.getAttributeBaseValue(EntityAttributes.GENERIC_MOVEMENT_SPEED);
        ...
        // scale not supported
        setAttribute(zombie, EntityAttributes.GENERIC_MOVEMENT_SPEED, newSpeed);
        ...
        double dBase = zombie.getAttributeBaseValue(EntityAttributes.GENERIC_ATTACK_DAMAGE);
        setAttribute(zombie, EntityAttributes.GENERIC_ATTACK_DAMAGE, dBase * newScale * healthBonus);
    }
}
