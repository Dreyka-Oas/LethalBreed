...
public class MutantLogic {
    public static void makeMutant(ZombieEntity z) {
        z.addScoreboardTag("lethal_mutant");
    }

    public static void tickTentacles(ZombieEntity z) {
        if (!z.getScoreboardTags().contains("lethal_mutant") || z.age % ModConfig.INSTANCE.mutant.mutantTentacleTickRate != 0) return;
    ...
    public static void onDeath(ZombieEntity z) {
        if (!z.getScoreboardTags().contains("lethal_mutant")) return;
        ...
        for (int i = 0; i < ModConfig.INSTANCE.mutant.mutantMinionCount; i++) {
            ...
            minion.removeScoreboardTag("lethal_mutant"); // Security fix: prevent infinite spawn loops
