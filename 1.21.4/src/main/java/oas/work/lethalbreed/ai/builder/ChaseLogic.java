...
public class ChaseLogic {
    public static int process(Zombie z, Vec3 tPos, AtomicReference<BlockPos> ab, AtomicReference<Boolean> th, int st, BuildStateMachine bsm, boolean hasVisualTarget) {
        ...
        if (hasVisualTarget && z.onGround()) {
            BlockPos obstruction = ObstructionAnalyzer.getHorizontal(z.level(), z, tPos);
            if (obstruction != null) {
        ...
        if (dy > 0.8 && ConstructionCoordinator.shouldClimb(z, tPos)) {
            ...
            if (!bp.equals(z.blockPosition())) z.moveTo(bp.getX() + 0.5, z.getY(), bp.getZ() + 0.5, z.getYRot(), z.getXRot());
            BlockPos lb = ObstructionAnalyzer.getObstruction(z.level(), bp, z);
        ...
        if (z.onGround() && MovementCoordinator.tryBuild(z, tPos)) {
            z.getNavigation().stop(); bsm.setGlobalCooldown(oas.work.lethalbreed.config.ModConfig.INSTANCE.movement.buildGlobalCooldownTicks); return 0;
        }
