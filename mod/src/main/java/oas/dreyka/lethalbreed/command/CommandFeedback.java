package oas.dreyka.lethalbreed.command;
...
final class CommandFeedback {
    ...
    /** The prefix stays a literal (it is a brand, not a word) and only the message is translated, so each
     *  recipient reads it in their own language instead of the server's. */
