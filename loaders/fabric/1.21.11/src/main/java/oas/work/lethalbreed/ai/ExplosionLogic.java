 ...
 * Responsibility: Explosion and Effect logic for Kamikaze Zombies
...
public class ExplosionLogic {
    public static void spawnParticles(ZombieEntity z, float bonus, int fuse) {
        if (z.getEntityWorld().isClient()) return;
        var world = (net.minecraft.server.world.ServerWorld)z.getEntityWorld();
    ...
    public static void detonate(ZombieEntity z, float bonus) {
        ...
        z.getEntityWorld().createExplosion(z, z.getX(), z.getY(), z.getZ(), power, World.ExplosionSourceType.MOB);
        z.discard();
    }
}
