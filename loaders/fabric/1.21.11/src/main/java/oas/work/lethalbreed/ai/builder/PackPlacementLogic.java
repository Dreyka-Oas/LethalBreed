 ...
 * Responsibility: Logic for Pack Positioning during Construction
...
public class PackPlacementLogic {
    public static BlockPos getBetterConstructionPos(ZombieEntity z) {
        ...
        for (BlockPos opt : options) {
            if (world.getBlockState(opt).isReplaceable() && world.getBlockState(opt.down()).isSolidBlock(world, opt.down())) {
                return opt;
            }
        }
        return base;
