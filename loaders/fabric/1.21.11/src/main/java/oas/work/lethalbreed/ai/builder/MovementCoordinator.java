 ...
 * Responsibility: Building and Movement Coordination
...
public class MovementCoordinator {
    public static boolean tryBuild(ZombieEntity z, Vec3d t) {
        ...
        BlockPos next = BlockPos.ofFloored(z.getX() + dir.x, z.getY() - 0.5, z.getZ() + dir.z);
        if (next.equals(curr)) return false;

        BlockPos floor = (t.y < z.getY() - 0.5) ? next.down(2) : next.down();
        if (w.getBlockState(floor).isReplaceable() && VoidAnalyzer.isDeep(w, floor)) {
